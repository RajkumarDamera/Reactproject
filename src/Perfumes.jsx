...
function Perfumes() {
  ...
  const PerfumeItems = useSelector((state) => state.Products.Perfumes);
  ...
  return (
    <div className="container my-4">
      ...
      <h2 className="text-center text-warning mb-4 fw-bold d-flex justify-content-center align-items-center gap-2">
        <GiPerfumeBottle size={36} /> Perfumes Collection
      </h2>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="d-flex justify-content-center mb-4 gap-2 flex-wrap">
            <button className="btn btn-outline-warning" onClick={() => handlePriceFilter(0, 100)}>0–100 Rs</button>
            <button className="btn btn-outline-warning" onClick={() => handlePriceFilter(101, 200)}>101–200 Rs</button>
            <button className="btn btn-outline-warning" onClick={() => handlePriceFilter(201, 500)}>201–500 Rs</button>
          ...
          <div className="row row-cols-1 row-cols-md-4 g-4">
            {currentItems.map((item) => (
              <div key={item.id} className="col">
                <div className="card perfume-card h-100 position-relative shadow-sm hover-card">
                  {/* Wishlist Icon */}
                  <button
                  ...
                  <img src={item.Imageurl} className="card-img-top perfume-img rounded-top" alt={item.name} />
                  <div className="card-body d-flex flex-column">
                    <h5 className="card-title fw-bold">{item.name}</h5>
                    <p className="card-text text-muted small">{item.Discription}</p>
                    <h6 className="text-warning fw-bold mb-3">₹ {item.price}</h6>
                    <button className="btn btn-warning mt-auto rounded-pill shadow-sm" onClick={() => handleAddToCart(item)}>
                      Add To Cart 🛒
                    </button>
                  </div>
                </div>
      ...
      <style>{`
        ...
        .perfume-img { height: 180px; object-fit: cover; }
